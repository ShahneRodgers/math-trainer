...
updateScore score wasCorrect currTime =
  if wasCorrect then
    { score
      | points = score.points + (convertTimeToPoints currTime score.startTime)
      , startTime = currTime
    }
  else
...
progressBar width height score =
  color gray (collage width height [ innerProgress (toFloat width) (toFloat height) score ])
